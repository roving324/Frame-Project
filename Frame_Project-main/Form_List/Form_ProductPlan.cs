...
namespace Form_List
{
	public partial class Form_ProductPlan : Assambl.Base_Form
	{
		...
		private void Form_ProductPlan_Load(object sender, EventArgs e)
		{
			...
			_GridUtil.InitColumnUltraGrid(grid, "ORDERCLOSEFLAG", "지시종료여부", GridColDataType_emu.CheckBox  , 120, HAlign.Left  , true, false);
				...
				foreach (DataRow drRow in dtTemp.Rows)
				{
					switch (drRow.RowState)
					{
						...
						case DataRowState.Modified:
							if (Convert.ToString(drRow["CHK"]) == "0") i = "FALSE";
                            else
                            {
                                if (Convert.ToBoolean(drRow["CHK"])) i = "TRUE";
                                else i = "FALSE";
                            }
                            if (Convert.ToString(drRow["ORDERCLOSEFLAG"]) == "") j = "FALSE";
                            else
                            {
                                if (Convert.ToBoolean(drRow["ORDERCLOSEFLAG"])) j = "TRUE";
                                else j = "FALSE";
                            }
                            helper.ExecuteNoneQuery("PRODUCT_PLAN_I", "@PLANTCODE", Convert.ToString(drRow["PLANTCODE"]), "@ITEMCODE", Convert.ToString(drRow["ITEMCODE"]), "@WORKCENTERCODE", Convert.ToString(drRow["WORKCENTERCODE"]), "@ID", common.sID, "@CHK", i, "@ORDERCLOSEFLAG", j, "@PLANNO", Convert.ToString(drRow["PLANNO"]));
							break;
					}
				}
				helper.Commit();
